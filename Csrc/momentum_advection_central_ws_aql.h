...
// (A): d1=d2 --> normal flux; d1!=d2 --> QL flux
void fourth_order_ws_m_aql_A(struct DimStruct *dims, double* restrict rho0, double* restrict rho0_half,
    ...
    double* restrict tendency, ssize_t d_advected, ssize_t d_advecting){
        ...
        if (d_advected==1 && d_advecting==1){
            printf("4th order WS AQL Momentum Transport: adding vel. variance transport \n");}
...
// (B): d_advecting=2 (vertical transport) --> normal flux; d_advecting!=2 (horizontal transport) --> QL flux
void fourth_order_ws_m_aql_B(struct DimStruct *dims, double* restrict rho0, double* restrict rho0_half,
    double* restrict alpha0, double* restrict alpha0_half,
    double* restrict vel_advected, double* restrict vel_advecting,
    double* restrict tendency, ssize_t d_advected, ssize_t d_advecting){

        if (d_advected==1 && d_advecting==1){
            printf("4th order WS AQL Momentum Transport: adding vertical transport \n");}
...
// (AB): d1=d2 || d1=2 --> normal flux; d1!=d2 && d1!=2 --> QL flux
void fourth_order_ws_m_aql_AB(struct DimStruct *dims, double* restrict rho0, double* restrict rho0_half,
    double* restrict alpha0, double* restrict alpha0_half,
    double* restrict vel_advected, double* restrict vel_advecting,
    double* restrict tendency, ssize_t d_advected, ssize_t d_advecting){

        if (d_advected==1 && d_advecting==1){
            printf("4th order WS AQL Momentum Transport: adding TKE transport and vertical momentum fluxes \n");}
...
//                                 interp_4(vel_advected[ijk+sm1_ed],vel_advected[ijk],vel_advected[ijk+sp1_ed],vel_advected[ijk+sp2_ed])) * rho0[k];
        ...
        // (5) compute QL flux: flux = flux - eddy_flux + mean_eddy_flux
        if (d_advecting != d_advected && d_advecting != 2){
            for(ssize_t i=imin;i<imax;i++){
                const ssize_t ishift = i*istride;
                for(ssize_t j=jmin;j<jmax;j++){
                    const ssize_t jshift = j*jstride;
                    for(ssize_t k=kmin;k<kmax;k++){
                        const ssize_t ijk = ishift + jshift + k;
                        flux[ijk] = flux[ijk] - eddy_flux[ijk] + mean_eddy_flux[k];
//                        if (i == kmin && j == jmin && k == kmax-1){printf("d1=%zd, d2=%zd: QL flux\n", d_advecting, d_advected);}
                    }
                }
            }
        }
...
// (D): d_advecting!=2 (horizontal transport) --> normal flux; d_advecting=2 (vertical transport) --> QL flux
void fourth_order_ws_m_aql_D(struct DimStruct *dims, double* restrict rho0, double* restrict rho0_half,
    double* restrict alpha0, double* restrict alpha0_half,
    double* restrict vel_advected, double* restrict vel_advecting,
    double* restrict tendency, ssize_t d_advected, ssize_t d_advecting){

        if (d_advected==1 && d_advecting==1){
            printf("4th order WS AQL Momentum Transport: adding horizontal momentum transport \n");}
